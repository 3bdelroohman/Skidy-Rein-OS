...
export default function StudentNotesInline({ groupId, studentId }: Props) {
  ...
  function handleAdd() {
    ...
    setDraft("");
    reload();
    toast.success(t(locale, "ØªÙ…Øª Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…Ù„Ø§Ø­Ø¸Ø©", "Note added"));
  }

  function handleDelete(id: string) {
    const ok = window.confirm(t(locale, "Ø­Ø°Ù Ù‡Ø°Ù‡ Ø§Ù„Ù…Ù„Ø§Ø­Ø¸Ø©ØŸ", "Delete this note?"));
    if (!ok) return;
    deleteStudentNote(id);
    reload();
    toast.success(t(locale, "ØªÙ… Ø­Ø°Ù Ø§Ù„Ù…Ù„Ø§Ø­Ø¸Ø©", "Note deleted"));
  }
  ...
  return (
    <div className="mt-2 w-full">
      ...
      >
        <StickyNote size={13} />
        {t(locale, "Ù…Ù„Ø§Ø­Ø¸Ø§Øª Ø§Ù„Ø£Ø¯Ø§Ø¡", "Progress notes")}
        {notes.length > 0 && (
        ...
        <div className="mt-2 rounded-xl border border-border bg-muted/30 p-3">
          ...
          <div className="mb-3 flex gap-2">
            <input
              ...
              placeholder={t(locale, "Ø£Ø¶Ù Ù…Ù„Ø§Ø­Ø¸Ø© Ø¹Ù„Ù‰ Ø£Ø¯Ø§Ø¡ Ø§Ù„Ø·Ø§Ù„Ø¨...", "Add a note about student performance...")}
          ...
          {notes.length === 0 ? (
            <p className="py-3 text-center text-xs text-muted-foreground">
              {t(locale, "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ù„Ø§Ø­Ø¸Ø§Øª Ø¨Ø¹Ø¯", "No notes yet")}
            </p>
          ) : (
