                  ...
                  <tbody>
                    {data.salesPerformance.map((item) => (
                      <tr key={item.name} className="border-b border-border last:border-0">
                        ...
                        <td className="px-4 py-3 font-bold text-foreground">{locale === "ar" ? item.revenue.toLocaleString("en-US") : item.revenue.toLocaleString("en-US")} {t(locale, "Ø¬.Ù…", "EGP")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function formatMoney(value: number, locale: "ar" | "en") {
  return `${locale === "ar" ? value.toLocaleString("en-US") : value.toLocaleString("en-US")} ${t(locale, "Ø¬.Ù…", "EGP")}`;
}
