...
export function getTeacherName(teacher: TeacherCourseLike | null | undefined): string {
  if (!teacher) return "";
  return teacher.fullName || teacher.full_name || teacher.name || "مدرس بدون اسم";
}
...
export function buildTeacherCourseWarning(input: {
  ...
  const availableNames = availableTeachers.map(getTeacherName).filter(Boolean).join("، ");

  if (locale === "ar") {
    return availableNames
      ? `المدرس ${teacherName} لا يدرّس مادة ${courseName}. يمكنك اختيار: ${availableNames}، أو تغيير المادة.`
      : `المدرس ${teacherName} لا يدرّس مادة ${courseName}. غيّر المادة أو أضف مدرسًا لهذه المادة.`;
  }
