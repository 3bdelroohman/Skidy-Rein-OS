...
type PaymentMoneyLike = {
  amount: number;
  currency?: "EGP" | "SAR" | null;
};

function formatPaymentAmount(payment: PaymentMoneyLike, locale: "ar" | "en"): string {
  return formatCurrency(payment.amount, locale, payment.currency ?? "EGP");
}

function formatPaymentTotals(items: PaymentMoneyLike[], locale: "ar" | "en"): string {
  if (items.length === 0) return formatCurrency(0, locale, "EGP");

  const totals = new Map<"EGP" | "SAR", number>();
  ...
  return Array.from(totals.entries())
    .map(([currency, amount]) => formatCurrency(amount, locale, currency))
    .join(" · ");
}
...
export default function PaymentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const siblingTotals = useMemo(() => formatPaymentTotals(payment?.siblingPayments ?? [], locale), [payment, locale]);
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <InfoRow label={t(locale, "المبلغ", "Amount")} value={formatPaymentAmount(payment, locale)} align={isAr ? "left" : "right"} />
                ...
                {payment.paymentHistory.map((item) => {
                  ...
                  return (
                    <Link key={item.id} href={`/payments/${item.id}`} className="flex items-center justify-between gap-3 rounded-2xl border border-border p-3 transition-colors hover:bg-muted/30">
                      <div>
                        <p className="font-semibold text-foreground">{formatPaymentAmount(item, locale)}</p>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="space-y-3">
              ...
              <InfoRow label={t(locale, "قابل للتحصيل", "Collectible amount")} value={formatPaymentAmount(payment, locale)} align={isAr ? "left" : "right"} />
              <InfoRow label={t(locale, "مدفوعات مرتبطة بالأسرة", "Family related payments")} value={siblingTotals} align={isAr ? "left" : "right"} />
                ...
                {payment.siblingPayments.slice(0, 4).map((item) => (
                  <Link key={item.id} href={`/payments/${item.id}`} className="block rounded-2xl border border-border p-3 transition-colors hover:bg-muted/30">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-sm font-bold text-foreground">{formatPaymentAmount(item, locale)}</span>
                    </div>
                  </Link>
                ))}
              </div>
