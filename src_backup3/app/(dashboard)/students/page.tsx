    ...
    return (
      <div className="space-y-6">
        <LoadingState
          ...
          descriptionAr="Ù†Ù‚Ø±Ø£ Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ø·Ù„Ø§Ø¨ Ù…Ù† Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øªâ€¦"
          descriptionEn="Fetching students from the databaseâ€¦"
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={t(
          locale,
          totals.total + " Ø·Ø§Ù„Ø¨ Ù…Ø³Ø¬ÙŽÙ‘Ù„ ÙÙŠ Ø§Ù„Ù†Ø¸Ø§Ù…",
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard
          tone="brand"
          label={t(locale, "Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø·Ù„Ø§Ø¨", "Total")}
          value={totals.total}
          icon={<GraduationCap className="size-4" />}
        />
        <StatCard
          tone="info"
          label={t(locale, "Ù…Ø­ØªÙ…Ù„ÙˆÙ† Ù…Ø±Ø´ÙŽÙ‘Ø­ÙˆÙ†", "Projected")}
          value={totals.projected}
          icon={<Sparkles className="size-4" />}
        />
        <StatCard
          tone="success"
          label={t(locale, "Ù†Ø´Ø·ÙˆÙ†", "Active")}
          value={totals.active}
          icon={<UserCheck className="size-4" />}
        />
        <StatCard
          tone="warning"
          label={t(locale, "Ø¨Ù„Ø§ Ù…Ø³Ø¤ÙˆÙ„", "Unassigned")}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <SearchBar
          value={search}
          onChange={setSearch}
          placeholder={t(locale, "Ø§Ø¨Ø­Ø« Ø¨Ø§Ø³Ù… Ø§Ù„Ø·Ø§Ù„Ø¨ Ø£Ùˆ ÙˆÙ„ÙŠ Ø§Ù„Ø£Ù…Ø±â€¦", "Search by student or parentâ€¦")}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <option value="all">{t(locale, "ÙƒÙ„ Ø§Ù„Ø­Ø§Ù„Ø§Øª", "All statuses")}</option>
      ...
      {!hasResults ? (
        <EmptyState
          ...
          title={
            hasFilters
              ? t(locale, "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù†ØªØ§Ø¦Ø¬ Ù…Ø·Ø§Ø¨Ù‚Ø©", "No matching results")
              : t(locale, "Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ø·Ù„Ø§Ø¨ Ø¨Ø¹Ø¯", "No students yet")
          }
          description={
            hasFilters
              ? t(locale, "Ø¬Ø±Ù‘Ø¨ ØªØ®ÙÙŠÙ Ø§Ù„ÙÙ„Ø§ØªØ± Ø£Ùˆ Ø§Ù„Ø¨Ø­Ø«.", "Try easing the filters or search.")
              : t(locale, "Ø§Ø¨Ø¯Ø£ Ø¨Ø¥Ø¶Ø§ÙØ© Ø£ÙˆÙ„ Ø·Ø§Ù„Ø¨.", "Add your first student.")
          }
          action={
            !hasFilters ? (
              <Button asChild>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="rounded-md bg-muted px-2 py-1.5">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                        {t(locale, "Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø©", "Group")}
                      </div>
                      <div className="truncate font-medium text-foreground">
                        {student.className ?? t(locale, "ØºÙŠØ± Ù…Ø³Ø¬Ù„", "Not assigned")}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                    {student.ownerName ? (
                      ...
                      <div className="rounded-md border border-dashed border-[var(--color-warning-200)] bg-[var(--color-warning-50)] px-2 py-1.5 text-[var(--color-warning-700)]">
                        <div className="text-[10px] uppercase tracking-wide opacity-70">
                          {t(locale, "ØªÙ†Ø¨ÙŠÙ‡", "Alert")}
                        </div>
                        <div className="font-medium">
                          {t(locale, "Ø¨Ù„Ø§ Ù…Ø³Ø¤ÙˆÙ„", "Unassigned")}
                        </div>
                      </div>
                    )}
                    <div className="rounded-md bg-muted px-2 py-1.5">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                        {t(locale, "Ø§Ù„Ø­Ø¶ÙˆØ±", "Attended")}
                      </div>
                      <div className="font-medium text-foreground" data-numeric>
                        {student.sessionsAttended} {t(locale, "Ø­ØµØ©", "sessions")}
                      </div>
                    </div>
                  </div>
