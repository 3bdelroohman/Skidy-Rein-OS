...
export default function GroupTasksPanel({ groupId }: Props) {
  ...
  function handleAdd() {
    ...
    setNotes("");
    reload();
    toast.success(t(locale, "ØªÙ…Øª Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…Ù‡Ù…Ø©", "Task added"));
  }
  ...
  function handleDelete(id: string) {
    const ok = window.confirm(t(locale, "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ù‡ Ø§Ù„Ù…Ù‡Ù…Ø©ØŸ", "Delete this task?"));
    if (!ok) return;
    deleteGroupTask(id);
    reload();
    toast.success(t(locale, "ØªÙ… Ø­Ø°Ù Ø§Ù„Ù…Ù‡Ù…Ø©", "Task deleted"));
  }

  const tabs: { key: TaskTargetType; labelAr: string; labelEn: string }[] = [
    { key: "teacher", labelAr: "Ù…Ù‡Ø§Ù… Ø§Ù„Ù…Ø¯Ø±Ø³", labelEn: "Teacher tasks" },
    { key: "student", labelAr: "Ù…Ù‡Ø§Ù… Ø§Ù„Ø·Ù„Ø§Ø¨", labelEn: "Student tasks" },
  ];
      ...
      {stats.total > 0 && (
        <div className="mb-4">
          <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
            <span>
              {stats.done} / {stats.total} {t(locale, "Ù…ÙƒØªÙ…Ù„", "done")}
            </span>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
          <div className="space-y-2">
            <input
              ...
              placeholder={t(
                locale,
                activeTab === "teacher" ? "Ø¹Ù†ÙˆØ§Ù† Ø§Ù„Ù…Ù‡Ù…Ø© Ù„Ù„Ù…Ø¯Ø±Ø³..." : "Ø¹Ù†ÙˆØ§Ù† Ø§Ù„Ù…Ù‡Ù…Ø© Ù„Ù„Ø·Ù„Ø§Ø¨...",
            ...
            <input
              ...
              placeholder={t(locale, "Ù…Ù„Ø§Ø­Ø¸Ø© Ø§Ø®ØªÙŠØ§Ø±ÙŠØ©...", "Optional note...")}
              onKeyDown={(e) => { if (e.key === "Enter") handleAdd(); }}
              className="w-full rounded-xl border border-input bg-card px-4 py-2.5 text-sm text-foreground focus:border-transparent focus:ring-2 focus:ring-ring"
            />
          </div>
      ...
      {tasks.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
          {t(
            locale,
            activeTab === "teacher"
              ? "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ù‡Ø§Ù… Ù„Ù„Ù…Ø¯Ø±Ø³ Ø¨Ø¹Ø¯"
              : "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ù‡Ø§Ù… Ù„Ù„Ø·Ù„Ø§Ø¨ Ø¨Ø¹Ø¯",
            activeTab === "teacher"
