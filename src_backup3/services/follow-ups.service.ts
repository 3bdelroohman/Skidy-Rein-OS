...
function mapRow(
  ...
  return {
    ...
    title: row.title || "Ù…ØªØ§Ø¨Ø¹Ø©",
    leadName: leadEntry?.childName || "Ø¹Ù…ÙŠÙ„ ØºÙŠØ± Ù…Ø­Ø¯Ø¯",
    parentName: leadEntry?.parentName || "ÙˆÙ„ÙŠ Ø£Ù…Ø± ØºÙŠØ± Ù…Ø­Ø¯Ø¯",
    type: asType(row.type),
    channel: asChannel(row.channel),
    priority: asPriority(row.priority),
    scheduledAt,
    status: isCompleted ? "completed" : resolveOpenStatus(scheduledAt),
    assignedTo: row.assigned_to ?? "ØºÙŠØ± Ù…Ø®ØµØµ",
  };
}
...
export async function createFollowUp(input: CreateFollowUpInput): Promise<FollowUpItem> {
  ...
  if (!supabase) {
    ...
    const typeLabel = item.type === "trial_reminder"
      ? "ØªØ°ÙƒÙŠØ± Ø¨Ø§Ù„Ø³ÙŠØ´Ù† Ø§Ù„ØªØ¬Ø±ÙŠØ¨ÙŠØ©"
      : item.title;

    await createLeadActivity(
      item.leadId,
      "ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ù…ØªØ§Ø¨Ø¹Ø© Ø¬Ø¯ÙŠØ¯Ø©: " + typeLabel,
      item.assignedTo,
      "contact",
    );
